...
class InferenceBlock(nn.Module):
    def __init__(self, inf_inp_dim, hidden_size, zsize, dropout_p, q_rnn_layers, dropout_locations, max_T):
        ...
        self.scaling_factor = nn.Parameter(torch.zeros(zsize))
    ...
    def sample_q_z(self, inf_inp, cond_inp, ELBO_samples):
        """
            output is z [T, B, s, E]
        """
        ...
        scaling_fac = self.scaling_factor.exp().view([1, 1, z_base_logvar.size(-1)])
        z_base_logvar = torch.tanh(z_base_logvar / scaling_fac.clamp(min=1.0)) * scaling_fac
...
class GenerativeBlock(nn.Module):
    ...
    def apply_bilstm(self, z, cond_inp_s):
        """
            z is [T, B, s, E]
        """
...
class DFModel(nn.Module):
    def __init__(self, vocab_size, loss_weights, n_inp_embedding, hidden_size, zsize, dropout_p, dropout_locations, # general parameters
                 prior_type, gen_bilstm_layers,q_rnn_layers, tie_weights, max_T, indep_bernoulli, gen_kwargs,
                 x_cond=False, y_cond=False, checkpoint_res=0): # misc parameters
        super().__init__()
        ...
        ## Latent models
        self.generative_model = GenerativeBlock(hidden_size, zsize, prior_type, dropout_p, dropout_locations, gen_bilstm_layers, max_T, **gen_kwargs)
    ...
    def generate(self, n_samples, temp=1.0, argmax_x=True):
        ...
        gen_outp = self.generative_model.apply_bilstm(z, cond_inp_s=pos_cond)
    ...
    def postprocess(self, x):
        ...
        assert (0 <= x).all() and (x < self.vocab_size).all()

        return torch.transpose(x, 0, 1)

    def evaluate_x(self, x, hps, kl_weight=0):
        """
        ...
        pos_cond = make_pos_cond(T, B, lengths, self.max_T)
        pos_cond_s = pos_cond[:, :, None, :].repeat(1, 1, hps.ELBO_samples, 1).view(T, B*hps.ELBO_samples, self.max_T*2)
        ...
        z, log_q_z = self.inference_model.sample_q_z(embeddings, pos_cond, hps.ELBO_samples) # [T, B, s, E]
            
        log_p_z = self.generative_model.prior.evaluate(z, cond_inp_s=pos_cond_s)
        gen_outp = self.generative_model.apply_bilstm(z, cond_inp_s=pos_cond_s)
        gen_outp = gen_outp.view(gen_outp.shape[0], B*hps.ELBO_samples, gen_outp.shape[-1]) # [T, B*s, 2*hidden]

        ## Final output
        scores = self.outp_ff(gen_outp) # [T, B, s, V]

        if self.indep_bernoulli:
            targets = x[:, :, None, :].repeat(1, 1, hps.ELBO_samples, 1)
            reconst_loss = self.cross_entropy(scores.view(-1, self.vocab_size), targets.view(-1, self.vocab_size)).view(T, B, hps.ELBO_samples, self.vocab_size).sum(-1) # [T, B, s]
        else:
            targets = x[:, :, None].repeat(1, 1, hps.ELBO_samples)
            reconst_loss = self.cross_entropy(scores.view(-1, self.vocab_size), targets.view(-1)).view(T, B, hps.ELBO_samples)
