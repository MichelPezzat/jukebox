...
class Affine():
    ...
    @staticmethod
    def get_pseudo_params(nn_outp, scaling_factor=None):
        a = nn_outp[..., 0] # [B, D]
        var_outp = nn_outp[..., 1]
        if scaling_factor is not None:
          scaling_fac = scaling_factor.exp().view(*(tuple([1 for _ in range(len(var_outp.shape)-1)])+scaling_factor.shape))
          var_outp = torch.tanh(var_outp / scaling_fac.clamp(min=1.0)) * scaling_fac
    ...
    @staticmethod
    def standard(x, nn_outp, scaling_factor=None):
        a, logbsq, b = Affine.get_pseudo_params(nn_outp, scaling_factor)
    ...
    @staticmethod
    def reverse(y, nn_outp, scaling_factor=None):
        a, logbsq, b = Affine.get_pseudo_params(nn_outp, scaling_factor)
...
class SCFLayer(nn.Module):
    def __init__(self, data_dim, n_hidden_layers, n_hidden_units, nonlinearity, transform_function, hidden_order=None, swap_trngen_dirs=False,
        ...
        self.scaling_factor = nn.Parameter(torch.zeros(data_dim-(data_dim//2)))
    ...
    def forward(self, inputs):
        ...
        x = torch.tensor(y)
        x[..., second_indices], change_logdet = self.train_func(y[..., second_indices], nn_outp, self.scaling_factor)
    ...
    def generate(self, inputs):
        ...
        y = torch.tensor(x)
        y[..., second_indices], change_logdet = self.gen_func(x[..., second_indices], nn_outp, self.scaling_factor)
