...
class LSTM_AFLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        y, logdet, hiddens, rnn_cond_inp = inputs # y is [T, B, inp_dim]
        ...
        return x, logdet, hiddens, rnn_cond_inp
    ...
    def generate(self, inputs):
        ...
        x, logdet, hiddens, rnn_cond_inp = inputs
        ...
        return y, logdet + change_logdet, hiddens, rnn_cond_inp
...
class LSTMFlow(nn.Module):
    ...
    def forward(self, y, hiddens, rnn_cond_inp=None):
        """
            Defines the reverse pass which is used during training
            logdet means log det del_y/del_x
        """
        ...
        if self.sequential_training:
            ...
            for flow_layer in reversed(self.flow):
                x, logdet, hiddens, _ = flow_layer.generate([x, logdet, hiddens, rnn_cond_inp])
        else:
            x, logdet, hiddens, _ = self.flow([y, logdet, hiddens, rnn_cond_inp])

        return x, logdet, hiddens

    def generate(self, x, hiddens, rnn_cond_inp=None):
        """
            Defines the forward pass which is used during testing
            logdet means log det del_y/del_x
        """
        ...
        if self.sequential_training:
            y, logdet, hiddens, _ = self.flow([x, logdet, hiddens, rnn_cond_inp])
        else:
            y = x
            for flow_layer in reversed(self.flow):
                y, logdet, hiddens, _ = flow_layer.generate([y, logdet, hiddens, rnn_cond_inp])
...
class AFPrior(nn.Module):
    ...
    def evaluate(self, z, cond_inp_s=None):
        """
            z is [T, B, s, E]
        ...
        eps, logdet, _ = self.flow(z, hidden, rnn_cond_inp=cond_inp_s)
    ...
    def generate(self, lengths, cond_inp=None, temp=1.0):
        ...
        z, logdet, _ = self.flow.generate(eps, hidden, rnn_cond_inp=cond_inp)
